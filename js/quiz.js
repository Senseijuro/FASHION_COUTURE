document.addEventListener('DOMContentLoaded', function() {
  ...
  Tutorial.show({
    ...
    subtitle: 'ÉPREUVE 2',
    description: 'En maroquinerie, chaque étape doit être réalisée dans un ordre très précis.',
    steps: [
      { icon: '👁️', text: 'Mémorise l\'ordre de fabrication affiché à l\'écran pendant 8 secondes.' },
      { icon: '🧩', text: 'Reconstruis ensuite l\'ordre exact en cliquant sur les différentes étapes.' },
      { icon: '❤️', text: 'Attention, tu n\'as droit qu\'à <strong>2 essais</strong> maximum !' }
    ],
  ...
  function initGame() {
    var allSequences = [
      { title: 'Fabrication d\'un sac en cuir', steps: [ { icon: '📐', text: 'Tracer et découper les pièces avec le gabarit' }, { icon: '🔪', text: 'Parer les bords au tranchet pour les amincir' }, { icon: '🪡', text: 'Percer les trous avec l\'alène' }, { icon: '🧵', text: 'Coudre les pièces au fil de lin ciré' }, { icon: '🪣', text: 'Teindre et finir les tranches de cuir' } ] },
      { title: 'Fabrication d\'un portefeuille', steps: [ { icon: '📏', text: 'Mesurer et tracer le patron sur le cuir' }, { icon: '✂️', text: 'Découper les pièces aux cisailles' }, { icon: '🔨', text: 'Aplanir le cuir au maillet sur le marbre' }, { icon: '🪡', text: 'Piquer les soufflets et les poches au fil' }, { icon: '✨', text: 'Lustrer et vernir pour la finition' } ] },
      { title: 'Fabrication d\'une ceinture', steps: [ { icon: '📐', text: 'Choisir et préparer le cuir selon l\'épaisseur' }, { icon: '🔪', text: 'Couper la lanière en bande régulière' }, { icon: '🔩', text: 'Percer les trous de réglage à l\'alène' }, { icon: '🪛', text: 'Fixer la boucle métal sur le bout de ceinture' }, { icon: '🪣', text: 'Colorer et cirer les bords pour la finition' } ] }
    ];
    ...
    correctOrder.forEach(function(step, i) {
      ...
      row.innerHTML = '<div class="seq-step-num">' + (i+1) + '</div><div class="seq-step-text">' + step.icon + ' ' + step.text + '</div>';
    ...
    function renderDropZone() {
      ...
      placed.forEach(function(stepIdx, pos) {
        ...
        el.innerHTML = '<div class="seq-placed-num">' + (pos+1) + '</div><span class="seq-placed-icon">' + step.icon + '</span><span class="seq-placed-text">' + step.text + '</span>';
    ...
    btnValidate.addEventListener('click', function() {
      ...
      var perfect = true;
      for (var i = 0; i < placed.length; i++) { if (placed[i] !== i) { perfect = false; break; } }
  ...
  function showResult(success) {
    ...
    if (success) {
      ...
      if (window.confetti) confetti({ particleCount: 150, spread: 80, origin: { y: 0.6 }, colors: ['#c8a96e', '#ff007f'] }); 
