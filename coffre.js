document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateDisplay() {
    ...
    if (state.enigme1 && state.enigme1.completed === true) {
      ...
      if (statusEnigme1) {
        statusEnigme1.textContent = 'Débloqué';
      ...
      if (statusEnigme1) {
        statusEnigme1.textContent = 'Échoué';
        statusEnigme1.classList.add('failed');
      }
    }

    // Epreuve 2 : Quiz Fashion
    ...
    if (state.quiz && state.quiz.completed === true) {
      ...
      if (statusQuiz) {
        statusQuiz.textContent = 'Débloqué';
      ...
      if (statusQuiz) {
        statusQuiz.textContent = 'Échoué';
        statusQuiz.classList.add('failed');
      }
    }

    // Epreuve 3 : Le Défilé Final
    ...
    if (state.enigma && state.enigma.completed === true) {
      ...
      if (statusEnigma) {
        statusEnigma.textContent = 'Débloqué';
      ...
      if (statusEnigma) {
        statusEnigma.textContent = 'Échoué';
    ...
    if (countCompleted === 3 && nextZone) {
      ...
      if (countSuccess === 3) {
        ...
        if (nextZoneTitle) nextZoneTitle.textContent = 'FÉLICITATIONS, DARLING !';
        if (nextZoneText) nextZoneText.textContent = 'Tu as débloqué tous les looks ! La collection est prête pour le podium !';
      } else if (countSuccess >= 1) {
        nextZone.classList.add('partial-zone');
        if (nextZoneTitle) nextZoneTitle.textContent = 'ÉPREUVES TERMINÉES';
        if (nextZoneText) nextZoneText.textContent = 'Tu as débloqué ' + countSuccess + '/3 looks, darling.';
      } else {
        nextZone.classList.add('fail-zone');
        if (nextZoneTitle) nextZoneTitle.textContent = 'ÉPREUVES TERMINÉES';
        if (nextZoneText) nextZoneText.textContent = 'Darling... la collection est un désastre. La rédactrice est furieuse.';
      }
    }
  ...
  document.querySelectorAll('.reward-card').forEach(function(card) {
    card.addEventListener('click', function() {
      ...
      if (gameState && gameState.completed === true) {
        ...
        if (failMessage) {
          failMessage.textContent = "Tu as échoué à l'épreuve, darling. Ce look reste verrouillé.";
        }
        if (modalFail) modalFail.classList.add('visible');
      } else {
        if (failMessage) {
          failMessage.textContent = "Tu dois d'abord compléter l'épreuve pour débloquer ce look, darling.";
        }
        if (modalFail) modalFail.classList.add('visible');
      }
    });
  ...
  if (btnReset) {
    btnReset.addEventListener('click', function() {
      if (confirm('Voulez-vous vraiment recommencer le jeu, darling ?')) {
        resetGameState();
        window.location.href = 'index.html';
      }
    });
